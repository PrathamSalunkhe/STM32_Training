...
#define USB_FLASH_START_ADDRESS   (  0x08000000 )    //USB Flash Address (Sector 0 in STM32L053)
...
//    uint32_t flash_addr = FLASH_BASE_ADDRESS + (blk_addr * STORAGE_BLK_SIZ);
//    for (uint16_t i = 0; i < blk_len; i++) {
//        memcpy(buf + i * STORAGE_BLK_SIZ, (uint8_t*)flash_addr, STORAGE_BLK_SIZ);
...
//    HAL_FLASH_Unlock();
//    for (uint16_t i = 0; i < blk_len; i++) {
//        for (uint32_t j = 0; j < STORAGE_BLK_SIZ; j += 8) {
//            uint64_t data = *(uint64_t*)(buf + i * STORAGE_BLK_SIZ + j);
//            HAL_FLASH_Program(FLASH_TYPEPROGRAMDATA_BYTE, flash_addr + j, data);
//        }
//        flash_addr += STORAGE_BLK_SIZ;
//    }
//    HAL_FLASH_Lock();
